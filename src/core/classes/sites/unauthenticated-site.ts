...
export class CoreUnauthenticatedSite {
    ...
    constructor(siteUrl: string, publicConfig?: CoreSitePublicConfigResponse) {
        this.siteUrl = CoreUrl.removeUrlParams(siteUrl); // Make sure the URL doesn't have params.
        if (publicConfig) {
    ...
    containsUrl(url?: string): boolean {
        ...
        const siteUrl = CoreTextUtils.addEndingSlash(CoreUrl.removeProtocolAndWWW(this.siteUrl));
        url = CoreTextUtils.addEndingSlash(CoreUrl.removeProtocolAndWWW(url));
    ...
    protected async requestPublicConfig(): Promise<CoreSitePublicConfigResponse> {
        ...
        if (config.httpswwwroot) {
            this.siteUrl = CoreUrl.removeUrlParams(config.httpswwwroot); // Make sure the URL doesn't have params.
        }
