...
describe('AsyncInstance', () => {
    ...
    it('does not initialize instance for eager methods', async () => {
        const asyncService = asyncInstance(() => new LazyService());
    ...
    it('preserves undefined properties', async () => {
        const asyncService = asyncInstance(() => new LazyService());

        // @ts-expect-error Accessing non-existing property for the first time.
        await expect(asyncService.thisDoesNotExist()).rejects.toBeInstanceOf(Error);

        // @ts-expect-error Accessing non-existing property for the second time.
