...
import {
    CORE_SITE_CURRENT_SITE_ID_CONFIG,
    CORE_SITE_SCHEMAS,
    CoreLoginSiteInfo,
    CoreRegisteredSiteSchema,
    CoreSiteBasicInfo,
    CoreSiteCheckResponse,
    CoreSiteSchema,
    CoreSitesLoginTokenResponse,
    CoreSitesLogoutOptions,
    CoreSitesReadingStrategy,
    CoreSiteUserTokenResponse,
    EagerCoreSitesProvider,
} from '@services/sites';
...
export class CoreSitesProvider extends EagerCoreSitesProvider {
    ...
    async isStoredRootURL(url: string, username?: string): Promise<{site?: CoreSite; siteIds: string[]}> {
        ...
        const siteUrl = CoreText.removeEndingSlash(
            CoreUrl.removeProtocolAndWWW(site.getURL()),
        );
        const treatedUrl = CoreText.removeEndingSlash(CoreUrl.removeProtocolAndWWW(url));
