...
export class CoreSearchHistoryProvider {
    ...
    protected searchHistoryTables: LazyMap<
        AsyncInstance<CoreDatabaseTable<CoreSearchHistoryDBRecord, 'searcharea' | 'searchedtext', never>>
    >;

    constructor() {
        this.searchHistoryTables = lazyMap(
            siteId => asyncInstance(
                () => CoreSites.getSiteTable<CoreSearchHistoryDBRecord, 'searcharea' | 'searchedtext', never>(
                    SEARCH_HISTORY_TABLE_NAME,
                    {
                        siteId,
                        primaryKeyColumns: ['searcharea', 'searchedtext'],
                        rowIdColumn: null,
