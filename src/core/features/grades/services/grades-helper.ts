...
export class CoreGradesHelperProvider {
    ...
    async getGradesCourseData(grades: any[]): Promise<any> {
        ...
        if (coursesWereMissing) {
            ...
            const coursesMap = CoreUtils.instance.arrayToObject(courses as { id: number }[], 'id');
    ...
    async goToGrades(
        ...
        try {
            ...
            try {
                ...
                const item = items.find((item) => moduleId == item.cmid);
                ...
                // @todo test that this work like before:
                // CoreContentLinksHelper.instance.goInSite(navCtrl, 'CoreGradesGradePage', pageParams, siteId)
                await CoreUtils.instance.ignoreErrors(CoreNavigator.instance.navigateToSitePath('/grades/grade', {
                    siteId,
                    params: pageParams,
                }));
            } catch (error) {
                ...
                if (userId && userId != currentUserId) {
                    ...
                    // @todo test that this work like before:
                    // CoreContentLinksHelper.instance.goInSite(navCtrl, 'CoreGradesCoursePage', pageParams, siteId)
                    await CoreUtils.instance.ignoreErrors(CoreNavigator.instance.navigateToSitePath('/grades/course', {
                        siteId,
                        params: pageParams,
                    }));
                }
            ...
            // @todo test that this work like before:
            // CoreContentLinksHelper.instance.goInSite(navCtrl, 'CoreGradesCoursePage', { course: { id: courseId } }, siteId))
            await CoreNavigator.instance.navigateToSitePath('/grades/course', {
                siteId,
    ...
    makeGradesMenu(
        ...
        if (gradingType > 0) {
            const grades: { label: string; value: string | number }[] = [];
            if (defaultLabel) {
