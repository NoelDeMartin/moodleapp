...
export class CoreUrl {
    ...
    static fixPluginfileURL(url: string, token: string, siteUrl: string, accessKey?: string): string {
        ...
        return CoreUrl.addParamsToUrl(url, { offline: '1' }); // Always send offline=1 (it's for external repositories).
    }
     ...
     */
    static getUrlScheme(url: string): string | void {
        if (!url) {
            return;
        }
        ...
        if (matches?.[1]) {
            ...
            switch (param) {
                ...
                case 'image':
                    ...
                    return CoreUrl.removeUrlParams(image);
                default:
     ...
     * @returns Treated URL.
     */
    static removeProtocolAndWWW(url: string): string {
        // Remove protocol.
        url = url.replace(/^.*?:\/\//, '');
        // Remove www.
        url = url.replace(/^www./, '');
