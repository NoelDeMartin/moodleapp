...
class performance_measure implements behat_app_listener {
    ...
    private function startDatabaseCount(): void {
        try {
            $this->databaseStart = $this->driver->evaluateScript('dbProvider.queryHistory.length') ?? 0;
        } catch (Exception $e) {
    ...
    private function analyseDatabaseUsage(): void {
        $this->database = $this->driver->evaluateScript('dbProvider.queryHistory.length') - $this->databaseStart;
    }
